...
def parse_ids_from_cell(cell):
    """FIXED: correct regex for digits."""
        ...
        for _, row in sub.iterrows():
            ...
            for citer in parse_ids_from_cell(row.get(CITED_BY)):
                if not citer or citer == target or citer not in valid_ids: 
                    continue
                cy = id_to_year.get(citer, np.nan)
                if not pd.isna(cy) and cy <= year:
                    m += 1
...
ax.plot(aus_df["Year"], aus_df["Density"], marker='o', linewidth=2.5, markersize=6, color='tab:purple', label='Australia')
ax.plot(uk_df["Year"],  uk_df["Density"],  marker='s', linewidth=2.5, markersize=6, color='tab:orange', label='United Kingdom')
